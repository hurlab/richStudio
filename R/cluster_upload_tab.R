sample_rr_path <- function(name) {
  ...
  if (!file.exists(fallback)) {
    stop("Sample file not found: ", name)
  }
  fallback
}

clusterUploadTabUI <- function(id, tabName) {
  ns <- NS(id)
      ...
      column(width = 4,
        tabBox(title=span(icon("upload"), "Upload enrichment results for clustering"), width=NULL,
          tabPanel("File upload",
            ...
            actionButton(ns('rr_load'), 'Load data')
          ),
          tabPanel("Text input",
...
clusterUploadTabServer <- function(id, u_degnames, u_degdfs, u_rrnames, u_rrdfs, u_big_rrdf, u_clusnames, u_clusdfs, u_big_clusdf, u_cluslists) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$rr_load, {
      # go1 and go2
      for (i in 1:2) {
        lab <- paste0('go', i)
        path <- sample_rr_path(paste0(lab, ".txt"))
        df <- read.delim(path)
    ...
    observeEvent(input$remove_clus, {
      ...
      for (clus in input$clus_list_table_rows_selected) {
        ...
        # Properly remove from reactive values using NULL assignment
        u_clusdfs[[clus_to_rm]] <- NULL
        u_cluslists[[clus_to_rm]] <- NULL
